...
void Display_7Seg(unsigned char *text, unsigned char decimal_point)
{
    static unsigned char enable_digit=1;

    if(enable_digit<=1)
    {
        if(decimal_point & 0b1000)
        {
            One_Digit_Handler(*text,1,1);
        }else
        {
            One_Digit_Handler(*text,1,0);
        }
        enable_digit=2;

    }else if(enable_digit==2)
    {
        if(decimal_point & 0b0100)
        {
            One_Digit_Handler(*(text+1),2,1);
        }else
        {
            One_Digit_Handler(*(text+1),2,0);
        }
        enable_digit=3;

    }else if(enable_digit==3)
    {
        if(decimal_point & 0b0010)
        {
           One_Digit_Handler(*(text+2),3,1);
        }else
        {
           One_Digit_Handler(*(text+2),3,0);
        }
        enable_digit=4;

    }else if(enable_digit>=4)
    {
        if(decimal_point & 0b0001)
        {
            One_Digit_Handler(*(text+3),4,1);
        }else
        {
            One_Digit_Handler(*(text+3),4,0);
        }
        enable_digit=1;
    }
...
void One_Digit_Handler(unsigned char sign, unsigned char digit_number, unsigned char decimal_point)
{
    ...
    switch (digit_number)
    {
        case 1 : PORTBbits.RB7=0; break;
        case 2 : PORTBbits.RB6=0; break;
        case 3 : PORTBbits.RB5=0; break;
